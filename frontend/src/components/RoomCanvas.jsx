    ...
    try {
      ...
      for (const room of rooms) {
        const roomRes = await axios.post(
          ...
          { project: projectId, name: room.name, width: room.width, height: room.height },
          { headers: { Authorization: `Bearer ${token}` } }
        )
        for (const item of furniture) {
          await axios.post(
            'http://127.0.0.1:8000/api/furniture/',
            { room: roomRes.data.id, name: item.name, x: item.x, y: item.y, width: item.width, height: item.height, color: item.fill },
            { headers: { Authorization: `Bearer ${token}` } }
          )
        }
      }
